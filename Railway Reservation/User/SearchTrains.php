...
<body>
    <div class="designClassBody">
        ...
        <?php
        if (!isset($_SESSION['pid'])) {
            echo "<h5 style='color: #bc1e1e; text-align: center; padding:3%; font-size: 1.7rem; font-weight: 650;
        }'>Login for Tickets</h5>";
        } else {
            echo "<div style='padding-top: 4%;
                font-size: 200%;
                text-align: center;
                font-weight: 500;'>Search Trains</div>";
        }
        ?>
        ...
        <form action="" method="POST" class="container-fluid filter_class">
            ...
            <div class="commom_source_dest">
                <button id="btnsearch" class="btn btn-sm btn-primary" name="btnsearch" style="font-weight: bold; background-color: #1658bb;border-color: #1658bb;">Search</button>
                <button id="btnshowall" class="btn btn-sm btn-primary" name="btnshowall" style="font-weight: bold; background-color: #1658bb;border-color: #1658bb;">Show Tickets</button>
            </div>
        </form>
                ...
                <tbody>
                    ...
                    ?>
                            ...
                            <tr>
                                ...
                                <?php if (isset($_SESSION['pid'])) { ?>
                                    <td>
                                        <button class="btn btn-sm btn-primary" style="font-weight: bold; background-color: #1658bb;border-color: #1658bb;" onclick="window.location.assign('Booking.php?fid=<?php echo $row[0]; ?>&departureDate=<?php echo $departureDate; ?>');">Book</button>
                                    </td>
                                <?php } ?>
                            </tr>
                    <?php }
        ...
        <?php
            ...
            if ($total_available_flights == 0) {
                echo "<script>alert('We are sorry but this train is not available...');</script>";
            }
        }
        ?>
        ...
        <div class="" style="font-weight: 600;">Total Available Trains on
            <?php
